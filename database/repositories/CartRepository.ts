...
export class CartRepository {
  private records: Map<string, ICart> = new Map();

  public async create(data: Omit<ICart, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<ICart> {
    const id = data.id || `cart-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  ...
  public async findMany(filter: Partial<ICart> = {}, page = 1, limit = 20): Promise<{ items: ICart[]; total: number; totalPages: number }> {
  ...
  public async update(id: string, updates: Partial<ICart>): Promise<ICart | null> {
    const entity = this.records.get(id);
    if (!entity || entity.isDeleted) return null;

    const updatedEntity: ICart = {
      ...entity,
      ...updates,
  ...
  public async count(filter: Partial<ICart> = {}): Promise<number> {
