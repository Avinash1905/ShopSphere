...
export class ReturnRepository {
  private records: Map<string, IReturn> = new Map();

  public async create(data: Omit<IReturn, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<IReturn> {
    const id = data.id || `return-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    const timestamp = new Date().toISOString();
    const entity: IReturn = {
      ...data,
      id,
      createdAt: timestamp,
      updatedAt: timestamp,
  ...
  public async findMany(filter: Partial<IReturn> = {}, page = 1, limit = 20): Promise<{ items: IReturn[]; total: number; totalPages: number }> {
  ...
  public async update(id: string, updates: Partial<IReturn>): Promise<IReturn | null> {
    const entity = this.records.get(id);
    if (!entity || entity.isDeleted) return null;

    const updatedEntity: IReturn = {
      ...entity,
      ...updates,
  ...
  public async count(filter: Partial<IReturn> = {}): Promise<number> {
