/**
 * ShopSphere Category Repository
...
export class CategoryRepository {
  private records: Map<string, ICategory> = new Map();

  public async create(data: Omit<ICategory, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<ICategory> {
    const id = data.id || `category-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
  ...
  public async findMany(filter: Partial<ICategory> = {}, page = 1, limit = 20): Promise<{ items: ICategory[]; total: number; totalPages: number }> {
  ...
  public async update(id: string, updates: Partial<ICategory>): Promise<ICategory | null> {
    const entity = this.records.get(id);
    if (!entity || entity.isDeleted) return null;
  ...
  public async count(filter: Partial<ICategory> = {}): Promise<number> {
