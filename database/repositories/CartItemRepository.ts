/**
 * ShopSphere CartItem Repository
...
export class CartItemRepository {
  private records: Map<string, ICartItem> = new Map();

  public async create(data: Omit<ICartItem, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<ICartItem> {
    const id = data.id || `cartitem-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    const timestamp = new Date().toISOString();
    const entity: ICartItem = {
      ...data,
      id,
      createdAt: timestamp,
      updatedAt: timestamp,
  ...
  public async findMany(filter: Partial<ICartItem> = {}, page = 1, limit = 20): Promise<{ items: ICartItem[]; total: number; totalPages: number }> {
  ...
  public async update(id: string, updates: Partial<ICartItem>): Promise<ICartItem | null> {
    const entity = this.records.get(id);
    if (!entity || entity.isDeleted) return null;

    const updatedEntity: ICartItem = {
      ...entity,
      ...updates,
  ...
  public async count(filter: Partial<ICartItem> = {}): Promise<number> {
