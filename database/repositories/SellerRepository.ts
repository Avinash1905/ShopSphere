...
export class SellerRepository {
  private records: Map<string, ISeller> = new Map();

  public async create(data: Omit<ISeller, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<ISeller> {
    const id = data.id || `seller-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    const timestamp = new Date().toISOString();
    const entity: ISeller = {
      ...data,
      id,
      createdAt: timestamp,
      updatedAt: timestamp,
  ...
  public async findMany(filter: Partial<ISeller> = {}, page = 1, limit = 20): Promise<{ items: ISeller[]; total: number; totalPages: number }> {
  ...
  public async update(id: string, updates: Partial<ISeller>): Promise<ISeller | null> {
    const entity = this.records.get(id);
    if (!entity || entity.isDeleted) return null;

    const updatedEntity: ISeller = {
      ...entity,
      ...updates,
  ...
  public async count(filter: Partial<ISeller> = {}): Promise<number> {
