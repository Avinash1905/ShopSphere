/**
 * ShopSphere OrderItem Repository
...
export class OrderItemRepository {
  private records: Map<string, IOrderItem> = new Map();

  public async create(data: Omit<IOrderItem, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<IOrderItem> {
    const id = data.id || `orderitem-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
    const timestamp = new Date().toISOString();
    const entity: IOrderItem = {
      ...data,
      id,
      createdAt: timestamp,
      updatedAt: timestamp,
  ...
  public async findMany(filter: Partial<IOrderItem> = {}, page = 1, limit = 20): Promise<{ items: IOrderItem[]; total: number; totalPages: number }> {
  ...
  public async update(id: string, updates: Partial<IOrderItem>): Promise<IOrderItem | null> {
    const entity = this.records.get(id);
    if (!entity || entity.isDeleted) return null;

    const updatedEntity: IOrderItem = {
      ...entity,
      ...updates,
  ...
  public async count(filter: Partial<IOrderItem> = {}): Promise<number> {
