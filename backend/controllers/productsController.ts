...
export class ProductsController {
  ...
  public async getById(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.json({
        success: true,
        message: `Successfully retrieved ${id} from products.`,
        data: { id }
      });
    } catch (err: any) {
      res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: err.message } });
  ...
  public async create(req: Request, res: Response): Promise<void> {
    try {
      res.status(201).json({
        success: true,
        message: 'Successfully created products resource.',
        data: req.body
      });
    } catch (err: any) {
  ...
  public async update(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.json({
        success: true,
        message: `Successfully updated ${id} in products.`,
        data: { id, ...req.body }
      });
    } catch (err: any) {
      res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: err.message } });
  ...
  public async remove(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.json({
        success: true,
        message: `Successfully removed ${id} from products.`
      });
    } catch (err: any) {
      res.status(500).json({ success: false, error: { code: 'INTERNAL_ERROR', message: err.message } });
    }
